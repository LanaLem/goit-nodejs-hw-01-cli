...
async function removeContact(contactId) {
    try {
        ...
        return console.log(
            `Contact with id: ${contactId} has been removed from list`
        );
    } catch (error) {
        console.error(error.message);
    }
...
async function addContact(name, email, phone) {
    try {
        ...
        if (contactsList.find(contact => contact.name === name)) {
            return console.log(`Contact witn name ${name} is already in list`);
        }

        if (contactsList.find(contact => contact.email === email)) {
            return console.log(`Contact witn email ${email} is already in list`);
        }
